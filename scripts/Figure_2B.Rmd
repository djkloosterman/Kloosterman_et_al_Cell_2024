...
## Pre-processing of VISIUM10x dataset and classification of spots
...
```{r}
load("~/surfdrive/Code Availability/Kloosterman_et_al/Kloosterman et al. 2022 - data availibility/GEO Data - processed/scrna-seq/gbm-complete-subset.Rda")

setwd("~/surfdrive/Code Availability/Kloosterman_et_al/Kloosterman et al. 2024 - data availibility/GEO Data - processed/pre-processed visium") # Set working directory to the raw data folder of the visium 10x data
...
S5_2 <- Load10X_Spatial(
  data.dir = paste(getwd(), "/S5_2", sep = ""),
  filename = "filtered_feature_bc_matrix.h5",
  assay = "Spatial",
  slice = "slice1",
  filter.matrix = TRUE,
...
S6_2 <- Load10X_Spatial(
  data.dir = paste(getwd(), "/S6_2", sep = ""),
  filename = "filtered_feature_bc_matrix.h5",
  assay = "Spatial",
  slice = "slice1",
  filter.matrix = TRUE,
...
S7_2 <- Load10X_Spatial(
  data.dir = paste(getwd(), "/S7_2", sep = ""),
  filename = "filtered_feature_bc_matrix.h5",
  assay = "Spatial",
  slice = "slice1",
  filter.matrix = TRUE,
...
gbm.merged_1 <- FindVariableFeatures(gbm.merged, verbose = FALSE)
...
S5_2$orig.ident <- c("S5_2")
S6_2$orig.ident <- c("S6_2")
S7_2$orig.ident <- c("S7_2")
S8_2$orig.ident <- c("S8_2")

gbm.merged <- merge(S1_2, c(S2_2, S3_2, S4_2, S5_2, S6_2, S7_2, S8_2))
rm(S1_2, S2_2, S3_2, S4_2, S5_2, S6_2, S7_2, S8_2)

# Add tumor information to each sample: Ink4a = PDG-Ink4a KO model, p53 = PDG-p53 KD model, Prim = Primary tumor, Rec = Recurrent tumor
names(gbm.merged@images) <- c("P53_Prim_S1_2", "P53_Rec_S2_2", "P53_Rec_S3_2", "P53_Rec_S4_2", "NF1_Prim_S5_2", "NF1_Prim_S6_2", "NF1_Prim_S7_2", "Ink4a_Prim_S8_2")
...
Cell_Signatures <- read_excel("signatures/Glioblastoma_meta-module_genelist.xlsx")
...
for (i in groups){
  ...
  gbm.merged@meta.data[which(fdscores@meta.data[,i]>0.25),paste("assignedto",i, sep="")] <- paste("assignedto",i, sep=" ")
...
x_abundance <- t(table( gbm.merged$orig.ident, gbm.merged$LLM))
write.csv(x_abundance, file = "~/Desktop/LLM.csv")
```
...
```{r}
Top30genes <- read_csv("~/surfdrive/Code Availability/Kloosterman_et_al/Kloosterman et al. 2022 - code availibility/signatures/Top30genes.csv")
...
M04.expression <- AverageExpression(gbm.combined_subset, assays = "RNA", features = M04, group.by = "macrovsrest")
M04.expression <- M04.expression$RNA
M04.expression <- as.data.frame(M04.expression)
...
B04.expression <- AverageExpression(gbm.combined_subset, assays = "RNA", features = B04, group.by = "macrovsrest")
B04.expression <- B04.expression$RNA
B04.expression <- as.data.frame(B04.expression)
...
M04 <- subset(rownames(M04.expression), subset = M04.expression$`rest` < 1)
B01 <-  subset(rownames(B01.expression), subset = B01.expression$`rest` < 1)
B02 <-  subset(rownames(B02.expression), subset = B02.expression$`rest` < 1)
B03 <-  subset(rownames(B03.expression), subset = B03.expression$`rest` < 1)
B04 <-  subset(rownames(B04.expression), subset = B04.expression$`rest` < 1)
```
...
MDMSubsetScoring <- function (object, 
                           B01.features,  B02.features, B03.features, B04.features
                           ,set.ident = FALSE, ctrl_genes = 100, ...)
    {
        name <- "MDM.Subset"
        ...
        colnames(x = cc.scores) <- c("rownames", "B01.Score", "B02.Score", "B03.Score",
                                     "MDM_subset")
        rownames(x = cc.scores) <- cc.scores$rownames
        cc.scores <- cc.scores[, c("rownames", "B01.Score", "B02.Score", "B03.Score", 
                                     "MDM_subset")]
        object[[colnames(x = cc.scores)]] <- cc.scores
        if (set.ident) {
            object[["old.ident"]] <- Idents(object = object)
